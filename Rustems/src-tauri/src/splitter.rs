...
fn model_is_cached() -> bool {
    let p = model_cache_path();
    p.exists() && std::fs::read_dir(&p)
...
pub async fn split_stems(input_path: String) -> Result<SplitResult, String> {
    ...
    // Rename outputs into device structure
    ...
    for (name, src) in &raw {
        ...
        // Encode WAV to MP3 (device requires MP3; WAV is ~50 MB per stem)
...
pub async fn check_splitter() -> Result<String, String> {
    ...
    Err("Model not downloaded yet".into())
}

// Downloads the model. Call this once on first launch (or when check_splitter fails).
#[tauri::command]
pub async fn download_model() -> Result<String, String> {
    task::spawn_blocking(|| {
        match stem_splitter_core::ensure_model("htdemucs_ort_v1", None) {
            Ok(handle) => Ok(format!("Downloaded to {}", handle.local_path.display())),
