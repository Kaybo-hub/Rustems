 ...
 // const CTRL_DELETE_ALBUM: u8  = 0x09; // DELETE_ALBUM
    ...
    const CTRL_WRITE_ALBUM: u8 = 0x08; // Prepare device to receive album write
    ...
    /// `extra_trailing_nulls`
    /// after the first `track-config` init JSON (so `}\\0\\0` before framing). Other uploads use 0.
    fn build_upload_init(metadata: &serde_json::Value, extra_trailing_nulls: u8) -> Vec<u8> {
        let mut json_bytes = serde_json::to_vec(metadata).unwrap();
        json_bytes.push(0x00);
        for _ in 0..extra_trailing_nulls {
            json_bytes.push(0x00);
        }
    ...
    /// Device JSON uses lowercase `t` + index for uploads (`"t1"`), while listings use `T1`.
    fn usb_track_lower(track_id: &str) -> String {
        let n = track_id
            .strip_prefix('T')
            .or_else(|| track_id.strip_prefix('t'))
            .unwrap_or(track_id);
        format!("t{}", n)
    }
    ...
    fn read_device_file(
        ...
        // NO ACK after DATA - the protocol spec and capture both confirm this
    ...
    fn sync_pass(
        ...
        for (album_id, track_ids) in listing {
            if album_id == RECORD_ALBUM {
                ...
                if is_pass2 {
                    ...
                    std::thread::sleep(Duration::from_millis(100)); // Give the firmware time to 'close' the file
                    
                    eprintln!("[sync] RECORD/T1: {:?}", t1.as_ref().map(|v| v.len()));
                    after_record = true;
                } else {
                    eprintln!("[sync] Pass 1 RECORD done, re-pinging...");
                    // Ensure the pipe is clear before the re-ping
                    drain_stale(handle, ep_in); 
        ...
        if is_pass2 && hit_record {
            // Second pass over non-RECORD albums (capture shows each album-config read twice)
            for (album_id, _) in listing {
    ...
    //  File upload 
    //
    // Upload protocol (verified from capture5.pcapng - official website):
    //   HOST → UPLOAD_INIT (MSG_FILE_HEADER + json + 0x00) - first track-config uses +1 extra 0x00
    //   DEV  → ACK [01 00 00]
    ...
    fn send_file(
        ...
        timeout: Duration,
        label: &str,
        extra_init_trailing_nulls: u8,
    ) -> Result<(), String> {
        metadata["size"] = serde_json::Value::from(body.len());

        handle.write_bulk(ep_out, &build_upload_init(&metadata, extra_init_trailing_nulls), timeout)
    ...
    fn do_upload(
        ...
        let result = do_upload_inner(&handle, ep_out, ep_in, track_name, stem_data, new_album);
        (handle, result)
    }
    ...
    fn do_upload_inner(
        ...
        // Storage check - done before entering sync/write mode to avoid disrupting protocol state
        ...
        eprintln!("[upload] listing_pkt raw: {:02x?}", &listing_pkt[..listing_pkt.len().min(32)]);
        ...
        // REQUIRED: double ping + delay to enter write mode
        eprintln!("[upload] Entering write mode...");

        // First ping
        let _ = do_ping(&handle, ep_out, ep_in, sync_timeout)?;
        drain_stale(handle, ep_in);

        // Small delay (CRITICAL)
        std::thread::sleep(std::time::Duration::from_millis(50));

        // Second ping (this flips the mode)
        let _ = do_ping(&handle, ep_out, ep_in, sync_timeout)?;
        drain_stale(handle, ep_in);
        ...
        // `new_album` (UI): create `A{n+1}` with first track `T1`. Otherwise append one track
        // to the highest-numbered album (`A{max}`) using the next global track index.
        let (album_id, track_id) = if new_album {
            let a = max_album_num + 1;
            eprintln!("[upload] New album requested → A{}, first track T1", a);
            (format!("A{}", a), "T1".to_string())
        } else {
            if max_album_num == 0 {
                return Err("No valid albums found on device".into());
            }
            (
                format!("A{}", max_album_num),
                format!("T{}", max_track_num + 1),
            )
        };

        eprintln!(
            "[upload] Allocated album: {}, track: {} (new_album={})",
            album_id, track_id, new_album
        );

        // Device is now primed - upload begins immediately after the final ping.

        let track_lower = usb_track_lower(&track_id);
        ...
        let mut track_body = serde_json::to_vec_pretty(&track_json).map_err(|e| e.to_string())?;
        track_body.push(0x00);

        let album_title = if new_album {
            track_name.as_str()
        } else {
            ""
        };
        let album_global_id = uuid::Uuid::new_v4().to_string();
        let album_json = serde_json::json!({
            "artist": "Unknown",
            "global_id": album_global_id,
            "id": &album_id,
            "title": album_title,
            "tracks": serde_json::Value::Null,
            "version": "1",
        });
        let mut album_body = serde_json::to_vec_pretty(&album_json).map_err(|e| e.to_string())?;
        album_body.push(0x00);

        if new_album {
            prepare_write_album(&handle, ep_out, ep_in, &album_id, audio_timeout)?;
        ...
        send_file(&handle, ep_out, ep_in,
            ...
            &track_body, audio_timeout, "track-config (init)", 1)?;
        ...
        send_file(&handle, ep_out, ep_in,
            ...
            &album_body, audio_timeout, "album-config (init)", 0)?;

        // Upload stems (large bulk transfers - use long timeout per chunk/ACK).
        for (stem_num, data) in &stem_data {
            ...
            send_file(&handle, ep_out, ep_in,
                ...
                data, audio_timeout, &format!("stem-{}", stem_num), 0)?;
            eprintln!("[upload] stem {} done", stem_num);
        }

        eprintln!("[upload] Finalizing upload (track-config / album-config)...");
        drain_stale(handle, ep_in);
        ...
        send_file(&handle, ep_out, ep_in,
            ...
            &track_body, audio_timeout, "track-config (commit 1)", 1)?;
        ...
        send_file(&handle, ep_out, ep_in,
            ...
            &album_body, audio_timeout, "album-config", 0)?;
        ...
        send_file(&handle, ep_out, ep_in,
            ...
            &track_body, audio_timeout, "track-config (commit 2)", 0)?;

        let record_json = serde_json::json!({
            "TrackColour": ["#ff0000", "#ff0000"],
            "tempos": [{ "time_ms": 0, "tempo_bpm": 120 }]
        });
        let mut record_body = serde_json::to_vec(&record_json).map_err(|e| e.to_string())?;
        record_body.push(0x00);
        send_file(&handle, ep_out, ep_in,
            serde_json::json!({
                "size": record_body.len(),
                "type": "track-config",
                "album": RECORD_ALBUM,
                "track": "T1",
                "stem": 0
            }),
            &record_body, audio_timeout, "track-config RECORD", 0)?;
    ...
    fn do_delete(
        ...
        // Device responds with [02 00 05 <ctrl_byte>] - do NOT ACK
        let resp = read_response(handle, ep_in, timeout)?;
        eprintln!("[delete] response: {:02x?}", &resp[..resp.len().min(8)]);
        if resp.len() < 4 || resp[2] != 0x05 || resp[3] != ctrl_byte {
            return Err(format!("Unexpected delete response: {:02x?}", &resp[..resp.len().min(8)]));
        }

        // Immediately ping - device returns updated listing with deleted item gone
        let listing_pkt = do_ping(handle, ep_out, ep_in, timeout)?;
        let listing = parse_track_listing(&listing_pkt);
        eprintln!("[delete] post-delete listing: {} albums", listing.len());

        // Read every album-config + all track-configs, then RECORD last
    ...
    fn prepare_write_album(
        ...
        let json = format!("{{\"album\":\"{}\"}}", album_id);
        let mut payload = vec![CTRL_WRITE_ALBUM];
        payload.extend_from_slice(json.as_bytes());
        payload.push(0x00);
        handle.write_bulk(ep_out, &build_message(MSG_CONTROL, &payload), timeout)
            .map_err(|e| format!("prepare_write_album failed: {}", e))?;
        let resp = read_response(handle, ep_in, timeout)?;
        eprintln!("[upload] prepare_write_album response: {:02x?}", &resp[..resp.len().min(6)]);
        // Expect [02 00 05 08]
        if resp.len() < 4 || resp[2] != 0x05 || resp[3] != CTRL_WRITE_ALBUM {
            return Err(format!("unexpected prepare_write response: {:02x?}", resp));
        }
        Ok(())
    }
    ...
    pub async fn upload_stems(
        ...
        for (i, name) in ["melody", "vocals", "bass", "drums"].iter().enumerate() {
            let actual_path = ["mp3", "wav"].iter().find_map(|ext| {
                let p = folder_path.join(format!("{}.{}", name, ext));
                if p.exists() { return Some(p); }
                if *name == "melody" {
                    let alt = folder_path.join(format!("other.{}", ext));
                    if alt.exists() { return Some(alt); }
                }
                None
            }).ok_or_else(|| format!("Failed to read stem '{}': no .mp3 or .wav found in {}", name, folder_path.display()))?;
            let data = fs::read(&actual_path).map_err(|e| format!("Failed to read stem '{}': {}", name, e))?;
        ...
        let (returned_handle, result) = task::spawn_blocking(move || {
            do_upload(handle, 0x01, 0x81, resolved_name, stem_data, new_album)
        })
        .await
